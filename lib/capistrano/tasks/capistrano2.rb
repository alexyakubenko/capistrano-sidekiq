Capistrano::Configuration.instance.load do
  ...
  namespace :sidekiq do
    ...
    def start_process(pid_file, idx, sidekiq_role)
      ...
      run "if [ -d #{current_path} ] && [ ! -f #{pid_file} ]; then cd #{current_path} ; #{fetch(:sidekiq_cmd)} #{args.compact.join(' ')} ; else echo 'Sidekiq is already running'; fi", pty: false, roles: sidekiq_role
    end
